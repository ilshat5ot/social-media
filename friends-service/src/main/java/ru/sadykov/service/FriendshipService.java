...
public interface FriendshipService {
     ...
     * @param targetUserId        - Идентификатор пользователя, которому необходимо отправить заявку, либо заявку от
     *                      которого необходимо одобрить.
     * @return              - Возвращает объект FriendshipDto, представляющий запись о добавленной дружбе.
     */
    FriendshipDto addAsFriend(Long currentUserId, Long targetUserId);
     ...
     * @param targetUserId      - Идентификатор пользователя, которого удаляют из друзей, либо отклоняют заявку в друзья.
     * @return              - Возвращает объект FriendshipDto, представляющий запись об удаление из друзей.
     */
    FriendshipDto deleteFromFriends(Long currentUserId, Long targetUserId);
}
